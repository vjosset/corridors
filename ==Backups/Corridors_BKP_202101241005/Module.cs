...
namespace Corridors
{
    ...
    public class Module
    {
        ...
        /// <summary>
        /// Gets the thumbnails matrix, with same indixes as the Images property (for mirror and rotation).
        /// </summary>
        public Image[,] Thumbnails { get; private set; } = new Image[4, 4];

        /// <summary>
        /// Gets the average color of this module (for MiniMap display).
        /// </summary>
        ...
        public void LoadImage()
        {
            if (!string.IsNullOrEmpty(this.ImagePath))
            {
                ...
                Image orig = Image.FromFile(this.ImagePath);
                ...
                // Build all the rotations/flips of the original image and thumbnails
                Image[] variations = new Image[8];
                Image[] thumbnailVariations = new Image[8];
                for (int i = 0; i < 8; i++)
                {
                    variations[i] = (Image)orig.Clone();
                    variations[i].RotateFlip((RotateFlipType)i);

                    thumbnailVariations[i] = (Image)this.Thumbnail.Clone();
                    thumbnailVariations[i].RotateFlip((RotateFlipType)i);
                }
                ...
                // Load the variations of the thumbnails for this module
                Thumbnails[0, 0] = thumbnailVariations[(int)RotateFlipType.RotateNoneFlipNone];
                Thumbnails[0, 1] = thumbnailVariations[(int)RotateFlipType.Rotate90FlipNone];
                Thumbnails[0, 2] = thumbnailVariations[(int)RotateFlipType.Rotate180FlipNone];
                Thumbnails[0, 3] = thumbnailVariations[(int)RotateFlipType.Rotate270FlipNone];

                Thumbnails[1, 0] = thumbnailVariations[(int)RotateFlipType.RotateNoneFlipX];
                Thumbnails[1, 1] = thumbnailVariations[(int)RotateFlipType.Rotate90FlipX];
                Thumbnails[1, 2] = thumbnailVariations[(int)RotateFlipType.Rotate180FlipX];
                Thumbnails[1, 3] = thumbnailVariations[(int)RotateFlipType.Rotate270FlipX];

                Thumbnails[2, 0] = thumbnailVariations[(int)RotateFlipType.RotateNoneFlipY];
                Thumbnails[2, 1] = thumbnailVariations[(int)RotateFlipType.Rotate90FlipY];
                Thumbnails[2, 2] = thumbnailVariations[(int)RotateFlipType.Rotate180FlipY];
                Thumbnails[2, 3] = thumbnailVariations[(int)RotateFlipType.Rotate270FlipY];

                Thumbnails[3, 0] = thumbnailVariations[(int)RotateFlipType.RotateNoneFlipXY];
                Thumbnails[3, 1] = thumbnailVariations[(int)RotateFlipType.Rotate90FlipXY];
                Thumbnails[3, 2] = thumbnailVariations[(int)RotateFlipType.Rotate180FlipXY];
                Thumbnails[3, 3] = thumbnailVariations[(int)RotateFlipType.Rotate270FlipXY];

                // Build the average color of this module (for minimap display)
                this.Color = Utils.GetImageMajorityColor(orig as Bitmap);
            }
        }

        private void LoadThumbnail(Image orig)
        {
