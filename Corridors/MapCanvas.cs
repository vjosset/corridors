...
namespace Corridors
{
    ...
    public class MapCanvas : Panel
    {
        ...
        {
            DrawModuleShadow(g, mod, Point.Empty);
        }

        private void DrawModuleShadow(Graphics g, MapModule mod, Point offset)
        {
            if (mod != null && !mod.IsInHand)
            {
                ...
                Rectangle drawRect = new Rectangle(pixelPos.X - offset.X, pixelPos.Y - offset.Y, mod.Width * this.Zoom, mod.Height * this.Zoom);
        ...
        private void DrawModule(Graphics g, MapModule mod, Point offset)
        {
            if (mod != null)
            {
                Point pixelPos = GetPixelPosFromTilePos(mod.Position);
        ...
        /// <summary>
        /// Renders this map as an image to the specified file.
        /// </summary>
        /// <param name="filePath"></param>
        public void ExportToImage(string filePath)
        {
            // Get the size of the map (in tile coordinates)
            Rectangle mapRect = new Rectangle();
            mapRect.Location = new Point(this.Map.AllModules.Min(mod => mod.X), this.Map.AllModules.Min(mod => mod.Y));
            mapRect.Width = this.Map.AllModules.Max(mod => mod.X + mod.Width) - mapRect.Location.X;
            mapRect.Height = this.Map.AllModules.Max(mod => mod.Y + mod.Height) - mapRect.Location.Y;

            // Get the size of the map (in pixel coordinates)
            Rectangle imgRect = new Rectangle(0, 0, mapRect.Width * Zoom, mapRect.Height * Zoom);

            // Prepare the offset
            Point offset = new Point(mapRect.Location.X * Zoom, mapRect.Location.Y * Zoom);

            // Prepare the output bitmap (full size)
            Bitmap output = new Bitmap(imgRect.Width, imgRect.Height);

            // Prepare the graphics object to draw onto the output bitmap
            Graphics g = Graphics.FromImage(output);

            // Draw onto the output bitmap
            foreach (MapLayer lay in this.Map.Layers)
            {
                if (lay.IsVisible)
                {
                    if (lay.ShowShadows)
                    {
                        // Draw the shadows for each module
                        foreach (MapModule mod in lay.Modules)
                        {
                            DrawModuleShadow(g, mod, offset);
                        }
                    }

                    // Draw the modules themselves
                    foreach (MapModule mod in lay.Modules)
                    {
                        DrawModule(g, mod, offset);
                    }
                }
            }

            // Now save the bitmap to the specified path
        ...
        public void MoveCurrentLayerUp()
        {
            if (this.CurrentMapLayerIndex > 0)
            {
                this.Map.Layers.Swap(this.CurrentMapLayerIndex, this.CurrentMapLayerIndex - 1);
                this.CurrentMapLayerIndex--;
            }
        }

        public void MoveCurrentLayerDown()
        {
            if (this.CurrentMapLayerIndex < this.Map.Layers.Count - 1)
            {
                this.Map.Layers.Swap(this.CurrentMapLayerIndex, this.CurrentMapLayerIndex + 1);
                this.CurrentMapLayerIndex++;
            }
